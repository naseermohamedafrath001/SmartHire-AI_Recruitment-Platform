...
const HRAssistant = () => {
  ...
  useEffect(() => {
    ...
    setMessages([{
      type: 'ai',
      message: "Welcome to SmartHire AI! I'm your intelligent recruitment assistant. I can help you analyze candidates, find the best fit for your roles, and answer deep insights about your candidate pool. How can I help you today?",
          ...
          {quickQuestions.map((item, index) => {
            ...
            return (
              <motion.button
                ...
                disabled={isLoading}
                className={`p-4 text-left rounded-xl border-2 border-gray-200 hover:border-primary-300 transition-all duration-300 bg-white ${isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-lg'
                  }`}
              >
                <div className="flex items-start space-x-3">
                  <motion.div
                    className={`p-3 rounded-lg ${item.color === 'primary' ? 'bg-gradient-to-br from-primary-100 to-primary-200' :
                        item.color === 'success' ? 'bg-gradient-to-br from-success-100 to-success-200' :
                          item.color === 'warning' ? 'bg-gradient-to-br from-warning-100 to-warning-200' :
                            item.color === 'purple' ? 'bg-gradient-to-br from-emerald-100 to-emerald-200' :
                              item.color === 'indigo' ? 'bg-gradient-to-br from-teal-100 to-teal-200' :
                                'bg-gradient-to-br from-gray-100 to-gray-200'
                      }`}
                    whileHover={{ rotate: [0, -5, 5, 0] }}
                    transition={{ duration: 0.3 }}
                  >
                    <Icon className={`h-5 w-5 ${item.color === 'primary' ? 'text-primary-600' :
                        item.color === 'success' ? 'text-success-600' :
                          item.color === 'warning' ? 'text-warning-600' :
                            item.color === 'purple' ? 'text-emerald-600' :
                              item.color === 'indigo' ? 'text-teal-600' :
                                'text-gray-600'
                      }`} />
                  </motion.div>
      ...
      >
        ...
        <div className="bg-gray-50 rounded-lg p-4 h-96 overflow-y-auto mb-4 space-y-4">
          <AnimatePresence>
            {messages.map((msg, index) => (
              ...
              >
                <div
                  className={`max-w-xs lg:max-w-2xl px-4 py-3 rounded-lg ${msg.type === 'user'
        ...
        >
          ...
          <motion.button
            ...
            className={`px-6 py-3 rounded-lg font-medium transition-all duration-200 flex items-center space-x-2 ${newMessage.trim() && !isLoading
